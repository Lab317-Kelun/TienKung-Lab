...
class AmpOnPolicyRunner:
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        # create buffers for logging AMP and task rewards
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    # ========== 处理终端状态转换 ==========
                    # 当环境重置时，需要将终端状态的AMP观察设置为重置后的状态
                    # 这样可以避免在重置时使用无效的AMP观察
                    ...
                    # ========== 原本的AMP奖励计算机制 ==========
                    # 1. predict_amp_reward() 函数的工作原理：
                    #    - 输入：当前AMP观察(amp_obs)、下一个AMP观察(next_amp_obs_with_term)、任务奖励(rewards)
                    #    - 过程：
                    #      a) 将当前状态和下一个状态拼接，通过判别器网络得到输出 d
                    #      b) 计算AMP奖励 = amp_reward_coef * clamp(1 - (1/4) * (d-1)^2, min=0)
                    #         (这个公式鼓励策略生成与专家数据相似的动作)
                    #      c) 如果 task_reward_lerp > 0，则将AMP奖励和任务奖励进行线性插值：
                    #         reward = (1 - task_reward_lerp) * AMP奖励 + task_reward_lerp * 任务奖励
                    #    - 输出：混合后的奖励（形状：[num_envs]）
                    # 
                    # 2. 问题：
                    #    - 即使命令速度为零，AMP奖励仍然会鼓励周期性运动
                    #    - 因为专家数据中包含行走动作，判别器会奖励任何与专家数据相似的动作
                    #    - 这导致机器人在静止时仍然会抬腿，而不是保持稳定
                    #
                    # 3. 解决方案：
                    #    - 当命令速度接近零时，大幅减少AMP奖励的权重
                    #    - 这样可以让任务奖励（包括静止时的关节位置/速度奖励）占主导地位
                    # 保存任务奖励（在计算AMP奖励之前）
                    task_rewards = rewards.clone()
                    rewards_amp_full = self.alg.discriminator.predict_amp_reward(
                        amp_obs, next_amp_obs_with_term, rewards, normalizer=self.alg.amp_normalizer
                    )[0]
                    
                    # ========== 根据命令速度调整AMP奖励 ==========
                    # 当命令速度接近零时，减少AMP奖励，避免鼓励周期性运动
                    if self.cfg.get("reduce_amp_reward_at_zero_velocity", False):
                        # 获取当前命令速度（线速度xy + 角速度z）
                        command = self.env.command_generator.command.to(self.device)
                        # 计算速度大小：sqrt(vx^2 + vy^2) + |vz|
                        command_speed = torch.norm(command[:, :2], dim=1) + torch.abs(command[:, 2])
                        # 速度阈值：0.1 m/s（当速度小于此值时视为静止）
                        nonzero_flag = command_speed > 0.1
                        
                        # 速度掩码：运动时保持完整AMP奖励，静止时减少到20%
                        # 这样在静止时，任务奖励（关节位置/速度奖励）会占主导地位
                        velocity_scale = nonzero_flag.float() + 0.2 * (~nonzero_flag).float()
                        rewards = rewards_amp_full * velocity_scale
                    else:
                        rewards = rewards_amp_full
                    ...
                    if self.log_dir is not None:
                        ...
                        # Update AMP and task rewards for logging
                        cur_amp_reward_sum += rewards_amp_full
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            # AMP and task rewards
            if "amp_rewbuffer" in locs and len(locs["amp_rewbuffer"]) > 0:
                self.writer.add_scalar("Amp/mean_amp_reward", statistics.mean(locs["amp_rewbuffer"]), locs["it"])
