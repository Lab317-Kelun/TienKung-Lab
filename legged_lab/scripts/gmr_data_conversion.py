...
def convert_pkl_to_custom(input_pkl, output_txt, fps):
    ...
    max_frames = min(1000, frames_data_len)
    print(f"👀 Data sequence length: {frames_data_len}, saving first {max_frames} frames")
    with open(output_txt, 'w') as f:
        ...
        for i, line in enumerate(frames_data[:max_frames]):
            line_start_str = '  ['
            if i == max_frames - 1:
