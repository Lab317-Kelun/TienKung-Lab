...
class MujocoRunner:
    ...
    def init_variables(self) -> None:
        ...
        1. MuJoCo 关节顺序 (from XML actuator section):
           MuJoCo 按照左腿、右腿、左臂、右臂的顺序排列：
            0: hip_roll_l_joint
            1: hip_pitch_l_joint
            2: hip_yaw_l_joint
        ...
        2. Isaac Lab 关节顺序 (from tienkung.py actuators):
           Isaac Lab 按照执行器组和关节类型排列：
           - "legs" 组: hip_roll, hip_pitch, hip_yaw, knee (每个关节先左后右)
           - "feet" 组: ankle_pitch, ankle_roll (每个关节先左后右)
           - "arms" 组: shoulder_pitch, shoulder_roll, shoulder_yaw, elbow (每个关节先左后右)
            0: hip_roll_l_joint
            1: hip_roll_r_joint
        ...
        3. 索引映射说明:
           - mujoco_to_isaac_idx: MuJoCo索引 -> Isaac Lab索引的映射
           - isaac_to_mujoco_idx: Isaac Lab索引 -> MuJoCo索引的映射
           例如: MuJoCo[0](hip_roll_l) -> Isaac Lab[0](hip_roll_l)
                 MuJoCo[1](hip_pitch_l) -> Isaac Lab[4](hip_pitch_l)
                 MuJoCo[6](hip_roll_r) -> Isaac Lab[1](hip_roll_r)
        """
