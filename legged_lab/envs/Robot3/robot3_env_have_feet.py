...
class Robot1_6Env(VecEnv):
    ...
    def visualize_motion(self, time):
        ...
        # 所有12个关节（左右腿各6个：hip_pitch, hip_roll, hip_yaw, knee, ankle_pitch, ankle_roll）
        ...
        # Return format: [right_leg_dof_pos(6), left_leg_dof_pos(6), right_leg_dof_vel(6), left_leg_dof_vel(6), right_foot_pos(3), left_foot_pos(3)] = 30维
        return torch.cat(
            (
    ...
    def get_amp_obs_for_expert_trans(self):
        """Gets amp obs from policy - includes all 12 joints (hip, knee, ankle) and feet positions."""
        # 计算脚部位置（相对于根节点的body坐标系）
        left_foot_pos = (
            self.robot.data.body_state_w[:, self.feet_body_ids[0], :3] - self.robot.data.root_state_w[:, 0:3]
        )
        ...
        # Output format: [right_leg_dof_pos(6), left_leg_dof_pos(6), right_leg_dof_vel(6), left_leg_dof_vel(6), right_foot_pos(3), left_foot_pos(3)] = 30维
        return torch.cat(
            (
                self.right_leg_dof_pos,   # 6
                self.left_leg_dof_pos,    # 6
