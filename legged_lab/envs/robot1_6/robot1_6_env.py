...
from legged_lab.envs.robot1_6.run_cfg import Robot1_6RunFlatEnvCfg
from legged_lab.envs.robot1_6.run_with_sensor_cfg import Robot1_6RunWithSensorFlatEnvCfg
from legged_lab.envs.robot1_6.walk_cfg import Robot1_6WalkFlatEnvCfg
...
class Robot1_6Env(VecEnv):
    def __init__(
        self,
        cfg: (
            Robot1_6RunFlatEnvCfg
    ...
    def init_buffers(self):
        ...
        # GMR 输出的关节顺序: hip_pitch, hip_roll, hip_yaw, knee, ankle_pitch, ankle_roll
        # 按照 GMR 输出顺序查找关节，preserve_order=True 确保返回的索引顺序与 name_keys 顺序一致
        self.left_leg_ids, _ = self.robot.find_joints(
            name_keys=[
                "left_hip_pitch_joint",   # GMR 顺序 [0]
                "left_hip_roll_joint",     # GMR 顺序 [1]
                "left_hip_yaw_joint",      # GMR 顺序 [2]
                "left_knee_joint",         # GMR 顺序 [3]
                "left_ankle_pitch_joint",  # GMR 顺序 [4]
                "left_ankle_roll_joint",   # GMR 顺序 [5]
            ],
            preserve_order=True,
        )
        self.right_leg_ids, _ = self.robot.find_joints(
            name_keys=[
                "right_hip_pitch_joint",   # GMR 顺序 [0]
                "right_hip_roll_joint",    # GMR 顺序 [1]
                "right_hip_yaw_joint",     # GMR 顺序 [2]
                "right_knee_joint",        # GMR 顺序 [3]
                "right_ankle_pitch_joint", # GMR 顺序 [4]
                "right_ankle_roll_joint",  # GMR 顺序 [5]
            ],
            preserve_order=True,
        )
        ...
        # For feet_clearance reward: cumulative foot height (similar to Gym version)
        self.feet_height = torch.zeros(
            self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False
        )
        self.last_feet_z = torch.full(
            (self.num_envs, 2), 0.05, dtype=torch.float, device=self.device, requires_grad=False
        )
    ...
    def visualize_motion(self, time):
        ...
        # Robot1_6 data format: [root_pos(3), euler(3), dof_pos(12), root_lin_vel(3), root_ang_vel(3), dof_vel(12)]
        # Total: 36 values
        # GMR 输出的关节顺序: hip_pitch, hip_roll, hip_yaw, knee, ankle_pitch, ankle_roll (每腿6个)
        # Joint positions: 6-18 (left leg: 6-12, right leg: 12-18)
        # Joint velocities: 24-36 (left leg: 24-30, right leg: 30-36)
        # self.left_leg_ids 和 self.right_leg_ids 已经按照 GMR 顺序排列，可以直接映射
        dof_pos[:, self.left_leg_ids] = visual_motion_frame[6:12]
        dof_pos[:, self.right_leg_ids] = visual_motion_frame[12:18]
        

        # Joint velocities start at index 24 (after root_pos(3) + euler(3) + dof_pos(12) + root_lin_vel(3) + root_ang_vel(3) = 24)
        dof_vel[:, self.left_leg_ids] = visual_motion_frame[24:30]
        dof_vel[:, self.right_leg_ids] = visual_motion_frame[30:36]
        ...
        # Root linear velocity is at index 18 (after root_pos(3) + euler(3) + dof_pos(12) = 18)
        lin_vel = visual_motion_frame[18:21].clone()
        ...
        # 只取前4个关节 (hip_pitch, hip_roll, hip_yaw, knee)，删除ankle
        # 注意：虽然我们设置了所有12个关节到仿真中，但AMP观测只包含hip和knee
        self.left_leg_dof_pos = dof_pos[:, self.left_leg_ids[:4]]
        self.right_leg_dof_pos = dof_pos[:, self.right_leg_ids[:4]]
        self.left_leg_dof_vel = dof_vel[:, self.left_leg_ids[:4]]
        self.right_leg_dof_vel = dof_vel[:, self.right_leg_ids[:4]]
        
        # Return format: [right_leg_dof_pos(4), left_leg_dof_pos(4), right_leg_dof_vel(4), left_leg_dof_vel(4)] = 16维
        # 只包含hip和knee，不包含ankle和foot位置
        return torch.cat(
            (
                self.right_leg_dof_pos,   # 4
                self.left_leg_dof_pos,    # 4
                self.right_leg_dof_vel,   # 4
                self.left_leg_dof_vel,    # 4
            ),
            dim=-1,
        )
    ...
    def reset(self, env_ids):
        ...
        # Reset feet clearance state
        self.feet_height[env_ids] = 0.0
        self.last_feet_z[env_ids] = 0.05
    ...
    def get_amp_obs_for_expert_trans(self):
        """Gets amp obs from policy - only includes hip and knee joints (no ankle or foot positions)."""
        # 只取前4个关节 (hip_pitch, hip_roll, hip_yaw, knee)，删除ankle
        ...
        # Output format: [right_leg_dof_pos(4), left_leg_dof_pos(4), right_leg_dof_vel(4), left_leg_dof_vel(4)] = 16维
        return torch.cat(
            (
                self.right_leg_dof_pos,   # 4
                self.left_leg_dof_pos,    # 4
