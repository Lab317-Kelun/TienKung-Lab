...
class LiteRewardCfg:
    ...
    joint_deviation_hip = RewTerm(
        ...
        weight=-0.15,  # 增加静止时的权重（从-0.15增加到-0.3）
        params={
    ...
    joint_deviation_legs = RewTerm(
        func=mdp.joint_deviation_l1,
        weight=-0.02,  # 增加静止时的权重（从-0.02增加到-0.1）
        params={
            "asset_cfg": SceneEntityCfg(
                "robot",
                joint_names=[
...
class Robot1_6WalkFlatEnvCfg:
    amp_motion_files_display = ["legged_lab/envs/robot1_6/datasets/motion_visualization/walk.txt"]
    device: str = "cuda:0"
...
class Robot1_6WalkAgentCfg(RslRlOnPolicyRunnerCfg):
    ...
    # amp parameter
    amp_reward_coef = 0.3
    amp_motion_files = ["legged_lab/envs/robot1_6/datasets/motion_amp_expert/walk.txt"]
